            ...
            {     
                ...
                if(fPath.length() != 0)
                {
                    ...
                    if(_t.joinable())
                    {
                        ...
                        _t = std::thread(&runFilter, std::ref(freq), std::ref(comp_gain), std::ref(madg_beta), std::ref(start_index), std::ref(end_index), std::ref(max_index), std::ref(gravity_vectors_madg_mag), std::ref(gravity_vectors_madg), std::ref(gravity_vectors_gt), std::ref(gravity_vectors_comp_mag), std::ref(gravity_vectors_comp));
                    }
                    else
                    {
                        // _t = std::thread(&runFilter);
